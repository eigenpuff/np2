	 ...
	 )
{
	...
	if(!file) { printf("File %s can't be opened\n",name); return(1); }
	fp->file=file;
	fp->buffer_index=0;
	fp->buffer_end=0;
	fp->bytesleft=0;
...
{	int err=0;
    ...
    if(!in) { printf("input file %s can't be opened\1",infile); err++; }
	else
	{
	FILE *out = fopen(outfile,"wb");
	if(!out) { printf("output file %s can't be opened\n",outfile); err++;}
	else
	{	BYTE buffer[BLOCKSIZE];
