    ...
    case CMD_TIMESTAMPS:
      {
        ...
        IniGetString(L"Settings2",L"TimeStamp",L"\\$Date: 2013/06/23 21:24:13 $]+\\$ | $Date: 2013/06/23 21:24:13 $",wchFind,COUNTOF(wchFind));
...
//
//
int FileIO(BOOL fLoad,LPCWSTR psz,BOOL bNoEncDetect,int *ienc,int *ieol,
...
BOOL FileLoad(BOOL bDontSave,BOOL bNew,BOOL bReload,BOOL bNoEncDetect,LPCWSTR lpszFile)
{
  ...
  {	// failed, but an error message was given
  }
  else if (!bFileTooBig)
...
BOOL FileSave(BOOL bSaveAlways,BOOL bAsk,BOOL bSaveAs,BOOL bSaveCopy)
{
  ...
  {	// failed, but warning message given
  }
